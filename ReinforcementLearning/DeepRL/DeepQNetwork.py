...
class Agent:
    def __init__(self, env, value, gamma=0.99, max_buffer_size=10000, min_buffer_size=100, batch_size=64):
        self.env = env
        self.gamma = gamma
        self.V = value
        self.state_buffer = []
    ...
    def play_one(self, e, target_network, max_iter=2000, update_freq=100, train=True):
        episode_reward = 0
...
def train(agent, target_network, episodes=1000, max_iter=1000, pritn_freq=1):
    ...
    for i in range(episodes):
        ...
        episode_reward = agent.play_one(target_network=target_network, e=eps, max_iter=max_iter, train=i%4==0)

        av_reward += episode_reward

        # display message
        if i % pritn_freq == 0:
            print("Episode {}, Total reward : {} |e:{}".format(i + 1, av_reward / pritn_freq, eps))
            av_reward = 0
...
def run_discrete_cartpole():
    ...
    train(agent, target_net, episodes=10000, pritn_freq=100, max_iter=2000)
