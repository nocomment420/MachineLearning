...
class Worker(mp.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            steps = []
class Worker3(mp.Process):
    def __init__(self, id, env_name, global_network, local_network, global_counter, max_total_steps, n_steps,
                 gamma, color):
    ...
    def train_global(self, steps):
        ...
        locs = list(self.local_network.parameters())
        globs = list(self.global_network.parameters())
        self.local_network.calc_loss(states, actions, returns)

        # copy gradient to global and apply gradient descent
        for (glob, loc) in zip(self.global_network.parameters(), self.local_network.parameters()):
            glob._grad = loc.grad.data.clone()
        torch.nn.utils.clip_grad_norm_(self.global_network.parameters(), 0.1)
        self.global_network.optim.step()

        # copy global params to local
        for (loc, glob) in zip(self.local_network.parameters(), self.global_network.parameters()):
            loc.data = glob.data.clone()
